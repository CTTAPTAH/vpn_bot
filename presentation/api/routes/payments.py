...
async def platega_webhook(requset: Request):
    ...
    if status == "CONFIRMED":
        ...
        use_case = ConfirmPaymentUseCase(build_uow(), get_vpn_gateway_factory())
        dto = await use_case.execute(PaymentProvider.PLATEGA, transaction_id)

        if dto.success:
            if dto.payment_action is PaymentAction.CREATE:
                text = texts.txt_purchase_success(dto.sub_token)
                keyboard = keyboards.kb_purchase_success(dto.sub_token)
            elif dto.payment_action is PaymentAction.RENEW:
                text = texts.txt_renew_success()
                keyboard = keyboards.kb_renew_success()
            else:
